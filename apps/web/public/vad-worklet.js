...
// микрофон молча гейтился в тишину (пиры не слышали, пока не вернёшься на вкладку).
//
// Лежит в public/ (а не в src/ с ?url): имя файла обязано пережить деплой. С хешированным именем
// вкладка, открытая ДО выкатки, после неё получала 404 на addModule — ворклет молча не поднимался,
// гейт откатывался на rAF, и фоновый баг возвращался ровно к тем, кто держит апп открытым сутками.
//
// Нода объявляет ОДИН выход (тишина) и подключается через gain=0 к destination: часть движков
// обрабатывает только ноды, достижимые от destination, и лист с numberOfOutputs:0 у них не
// пуллится вовсе. Выход мы не пишем — он остаётся нулевым, поэтому в звук ничего не примешивается.
