...
export class DiagnosticReportOutbox {
  ...
  flush(): void {
    ...
    void Promise.resolve().then(async () => {
      // Disposal may run after flush selected a report but before this microtask. In particular,
      // logout must not start a new request under a replacement account's ambient API token.
      if (!this.started || this.ownerDiscarded) return false;
      await this.upload(next.report);
      return true;
    }).then((uploaded) => {
      if (uploaded) this.remove(next);
    }, (error) => {
