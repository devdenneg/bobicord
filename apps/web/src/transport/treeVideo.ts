...
// Приёмный буфер зрителя 500мс (дефолт Chrome ~50мс). NACK-ретрансмит УЖЕ работает (webrtc-rs
// configure_nack активен на всех H.264-легах — верифицировано), но опоздавший за буфер пакет
// декодер выбрасывает и фризит до keyframe. Один NACK-цикл ≈ 100мс (тик Generator) + RTT;
// на лоссовом RU-леге (3.4% вечером) сам ретрансмит теряется → нужен 2-й цикл ≈ 250-400мс.
// 500мс впитывают ДВА цикла → NACK-восстановленный пакет успевает к воспроизведению, фриз не
// случается. Цена — +0.5с задержки (бюджет инварианта — «видео < 2с»). Ставим на ОБА приёмника
// (audio+video): буфер только у видео развёл бы губы со звуком.
const JITTER_TARGET_MS = 500;
