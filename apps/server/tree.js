...
const KF_FORWARD_MIN_MS = 1000;    // базовый rate-limit проброса request-keyframe к корню на ДЕРЕВО: IDR дорог
const KF_FORWARD_MAX_MS = 8000;    // потолок бэкоффа: при устойчивых потерях IDR только добивает линк (см. onRequestKeyframe)
const KF_BACKOFF_RESET_MS = 5000;  // столько тишины — петля PLI разорвалась, возвращаем базовый интервал
...
// Доля аплинка vrelay под видео. Остальное — аудио, RTCP, ICE, голос (livekit), веб (caddy):
// на одном VPS они делят тот же канал, и «использовать 100%» = очередь передачи ядра
// начинает дропать (см. serverBudget).
const VRELAY_BW_HEADROOM = 0.7;
...
class TreeManager {
  ...
  // Бюджет исходящей полосы серверного узла (vrelay): он отдаёт КАЖДОМУ прямому ребёнку
  // полный поток, поэтому его аплинк делится на число детей. Возвращает бит/с на ребёнка
  // либо null, если полоса неизвестна или детей нет.
  //
  // Зачем. Разбор прода: vrelay раздавал 4 зрителям по 4.7 Мбит ≈ 19 Мбит при РЕАЛЬНОМ
  // аплинке VPS 14-20 Мбит (замер; в env стояло 37 — цифра с параллельных TCP-потоков,
  // они съедают burst-кредит и врут вверх). Очередь передачи ядра выбросила 1.7М пакетов,
  // RTT до зрителя вырос с 29мс до 1.8с, все четверо разом ушли в 35-60% потерь.
  // Резать ЁМКОСТЬ (число детей) нельзя: браузерный зритель ретранслировать не умеет и
  // третьим стал бы сиротой без картинки. Режем битрейт — смотрят все, просто хуже.
  serverBudget(key) {
    const t = this.trees.get(key);
    const server = t && this.serverNode(key);
    if (!server || !server.children.length) return null;
    const out = server.availableOutgoing || 0;
    if (out <= 0) return null; // полоса не объявлена — не выдумываем
    return Math.floor((out * VRELAY_BW_HEADROOM) / server.children.length);
  }
  ...
  abrTick(streamId) {
    ...
    // Потолок по аплинку vrelay: он делится между прямыми детьми. AIMD по потерям реагирует
    // ПОСЛЕ того, как линк уже посыпался; этот потолок не даёт в него въехать. Пол ABR_FLOOR
    // уважаем — ниже него качество бессмысленно, лучше рвать зрителя репарентом.
    const budget = this.serverBudget(streamId);
    if (budget && target > budget) {
      target = Math.max(ABR_FLOOR, budget);
      if (!t.budgetLogged) { t.budgetLogged = true; tlog(`[${streamId}] цель ограничена аплинком vrelay: ${Math.round(target / 1000)} kbps`); }
    } else if (budget) {
      t.budgetLogged = false;
    }
...
function attachTreeServer(httpServer, opts) {
  ...
  function onJoin(id, msg) {
    ...
    if (parent) {
      // Свежий зритель не имеет ни одного декодируемого кадра: его request-keyframe законен
      // и не должен ждать бэкоффа PLI-петли (до KF_FORWARD_MAX_MS чёрного экрана). Сбрасываем
      // окно — следующий запрос пройдёт сразу, а петля, если она есть, раскрутится заново.
      const t = mgr.trees.get(key);
      if (t) { t.kfBackoffMs = KF_FORWARD_MIN_MS; t.lastKfForwardAt = 0; }
  ...
  function onRequestKeyframe(id) {
    ...
    // вещателю (нет IDR-шторма через деревья). Rate-limit — на КАЖДОМ дереве свой.
    //
    // Фиксированный интервал 1с оказался НЕ защитой, а полкой: разбор реального стрима дал
    // ровно 0.99 IDR/с все 10 минут (при GOP 4с ожидалось 0.25). Это положительная обратная
    // связь: зритель теряет пакет -> PLI -> корень форсит IDR -> крупный бурст пробивает
    // линк -> снова потери. Пока запросы идут подряд, разводим интервал вдвое до KF_MAX:
    // потери надо лечить снижением битрейта (ABR), а не подливать IDR.
    const now = Date.now();
    const last = t.lastKfForwardAt || 0;
    const gap = now - last;
    if (gap >= KF_BACKOFF_RESET_MS) t.kfBackoffMs = KF_FORWARD_MIN_MS; // тишина — петля разорвалась
    const wait = t.kfBackoffMs || KF_FORWARD_MIN_MS;
    if (last && gap < wait) return;
    t.kfBackoffMs = Math.min(KF_FORWARD_MAX_MS, wait * 2);
