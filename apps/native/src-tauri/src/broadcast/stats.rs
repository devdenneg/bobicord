...
pub struct SharedStats {
    ...
    /// Самый крупный кадр за окно и самый крупный keyframe, байты (оконные, читаются
    /// через swap(0)). Размер IDR — это и есть «бёрст»: 1080p на 4.5 Мбит даёт ~200-300 КБ,
    /// то есть ~170 пакетов подряд, полсекунды слабого канала. Средний битрейт этого не
    /// показывает: он один и тот же и при ровном потоке, и при залпах.
    pub frame_bytes_max: AtomicU64,
    pub key_bytes_max: AtomicU64,
}
