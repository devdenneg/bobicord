...
**Задача.** Когда у сирот нет живых relay-кандидатов (или зритель явно выбрал «через сервер»), в дерево джойнится headless-агент на VPS и **passthrough**-ретранслирует RTP (без транскода). Осознанное расширение исключения инварианта 1 (наравне с TURN). В отличие от TURN, разгружает аплинк вещателя: одна копия серверу, веер — с сервера.

**Реализация.**
- **relay-core** (`apps/relay-core`) — вынесенное из натива кросс-платформенное ядро relay-viewer (webrtc-rs, tokio, без tauri/windows); натив реэкспортирует `broadcast::{relay,signaling}` из него.
- **Агент** (`apps/relay`, бинарь `vrelay`, docker-сервис): control-WS к `/tree` (JWT HS256 uid `virtual-relay` из `SESSION_SECRET`), `vrelay-hello{capacity}` → сервер шлёт `vrelay-activate{streamId}` → сессия relay-core (`join{virtual:true}`), мультистрим. Env: `TREE_WS_URL`, `VRELAY_MAX_CHILDREN/IDLE_SEC/MAX_STREAMS/OUT_MBPS`.
- **Сервер** (`tree.js`): флагу `virtual` верит только JWT-uid `virtual-relay`; `VIRTUAL_COST=1000` в скоринге (живой пир всегда лучше); активация из `settleOrphans`; `ensureVirtualAttached` выселяет жертву из-под корня, если дерево забито; **дренаж** (R1 — мягкий увод детей на живые пиры, R2 — выселение `vrelay-release`, когда живой ёмкости хватает); release при обрушении дерева; ручной запрос — `request-reparent{targetParentId:'vrelay'}` (pin от дренажа до собственной миграции). Фикс: `pickParent`/ручной reparent не выбирают отвязанную от корня цель.
- **UI**: узел «🖥 Сервер» в панели дерева, бейдж «через сервер (fallback)», кнопка «Через сервер» (ручной фолбэк).

**AC.** (1) сирота без живых кандидатов получает видео через vrelay ≤ 5 c после активации; (2) при появлении живого relay ребёнок мигрирует с виртуала ≤ 2 дренаж-тиков (10 c), pinned — не мигрирует; (3) пустой виртуал уходит по `VRELAY_IDLE_SEC`; (4) при наличии живого кандидата виртуал не выбирается никогда; (5) уход вещателя мгновенно гасит сессию (release).

---

## 2. Порядок и проверка

- Идти строго Э0→Э9. Не начинать следующий, пока AC текущего не выполнены и не закоммичены.
