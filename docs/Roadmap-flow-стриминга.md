...
**Легенда статусов (единый вид):** 🟡 = код готов, автоматические проверки (компиляция/юнит/ad-hoc/
симулятор) зелёные, живой AC (латентность/CPU/полоса/реальный медиапуть/деплой на VPS) — на пользователе;
🟢 = живой AC снят; ⬜ = не начат. По состоянию на Д8 **все Д0–Д8 — 🟡** (см. сводную AC-таблицу в конце):
код и вся детерминированная автоматика зелёные, полный живой прогон медиапути снимается пользователем.
...
**Статус:** 🟡 код готов; node --check / tree-sim / d6-test / typecheck / cargo (все крейты + examples) зелёные; живой e2e (реальная рокировка на VPS, реальный upload зрителя) не проверен
...
### Выполнено:
- **`apps/server/tree.js` — динамическая ёмкость (считает СЕРВЕР).** `capacityOf` теперь для
  server-first ЗРИТЕЛЯ берёт `min(объявленная, dynamicCapacity)`. `dynamicCapacity(node)`:
  `out≥2×br×1.3 → 2` (ветвление 1→2 при доказанном запасе), `out≥1×br×1.3 → 1`, `out>0 → 0`
  (доказанно слабый — детей не даёт), `out≤0 → 1` (upload не измерен — консервативно, НЕ раздуваем
  на неизвестной/фейковой цифре). `br` = `watchedBitrate` (RENDITION_BITRATE рендишна или maxBitrate
  вещателя для source). Легаси/не-зритель → `null` → объявленная ёмкость (старое поведение цело,
  tree-sim зелёный). Объявленный `maxChildren` остался верхней границей честности + `MAX_CHILDREN_CAP`.
- **`arbitrateServerSlots(t)` — супер-сидеры (в тике abrTimer, после perViewerAbr).** Политика
  вынесена в чистый `mgr.planServerSlotSwap(key, now)` (для теста + исполнителя): если прямые слоты
  серверного узла (`mgr.serverNode` — vrelay в source / рендишн-корень) ЗАНЯТЫ и в глубине есть
  ЛИСТ-НАТИВ с upload ≥ `D6_SWAP_HYSTERESIS`(1.25)× худшего прямого ребёнка, вне cooldown, способный
  ретранслировать (`capacityOf≥1`, влезает по MAX_DEPTH) → план рокировки. Жертва — слабейший прямой
  ЛИСТ (или узел с наименьшим поддеревом, если листьев нет — не морозим чужое поддерево). Исполнитель
  поднимает кандидата штатным `mgr.reparent` ВНУТРИ того же рендишн-дерева, вытесняет жертву в глубину
  (`applyReparent`+`settleOrphans` сажает её под поднятого кандидата). **≤1 рокировки на дерево за тик**;
  cooldown `D6_SWAP_COOLDOWN_MS`(30с) на ОБА узла (реюз `reparentCooldownUntil`, общий с Д4-ABR).
- **`scoreParent` — вес upload.** `outBonus` в server-first ×`OUT_BONUS_WEIGHT_SF`(4) — при равной
  глубине сильный по upload выигрывает родительство явно; вклад держим ниже depthCost(100), латентность
  главнее. Легаси-скоринг не тронут.
- **Наследование качества (C) — структурно (Д3), меню под пиром выключено (Д4).** Рокировка идёт
  ВНУТРИ одного рендишн-дерева (тот же `treeKey`) → качество ребёнка = качество родителя без спец-кода;
  `qualityPinned` (Д4) рокировке не мешает (это пин КАЧЕСТВА, а не родителя). Проверено: не сломано.
- **Браузеры всегда листья (D).** `capacityOf` не-натив → 0 (Д0). `planServerSlotSwap` кандидатом
  берёт только натив (`if (!n.native) continue`) → браузер НИКОГДА не поднимается в relay-слот.
  Браузер-жертва вытесняется в глубину и остаётся листом (тест C2b подтверждает: ни один браузер не
  ретранслирует).
- **Клиент-натив — честный upload (снят фейк `8_000_000`).** `lib.rs::start_watch` больше НЕ
  фабрикует 8 Мбит: принимает `available_outgoing: Option<u32>` (дефолт 0). `native.ts::startNativeWatch`
  прокидывает; `treeVideo.ts::nativeWatch` берёт реальный upload из **кэша Д5-probe** (`getCachedProbe`,
  тот же механизм, что мерил вещатель) → бит/с; нет кэша → фоновый `measureUpload()` прогревает его к
  следующему watch (в текущую сессию не вносится — RelayConfig берёт значение при старте, картина не
  блокируется). 0 = не измерен → сервер даёт ёмкость 1.
- **`apps/server/d6-test.js` (НОВЫЙ) — ad-hoc тест.** Строит дерево прямо в возвращённом TreeManager
  (send() вне peers = no-op, WS не нужен), дёргает реальный `arbitrateServerSlots`. **24/24 PASS:**
  (C0) dynamicCapacity 20M→2 / 8M→1 / 1M→0 / 0→1, браузер cap 0; (C1 a) ровно одна рокировка + второй
  вызов 0; (C1 b) S↑ в слот vrelay, слабый D2↓ в глубину под S; (C1 g) cooldown 30с обоим (perViewerAbr
  их пропустит); (C2 c) сильный БРАУЗЕР в глубине НЕ поднят; (C2b) браузер-жертва↓ остаётся листом, ни
  один браузер не ретранслирует; (C3 d) сильный на 10% лучше → рокировки НЕТ; (C4 e) cooldown блокирует,
  по истечении — рокировка; (C5 f) без листьев вытесняется узел с МЕНЬШИМ поддеревом.
- **Верификация:** `node --check tree.js` OK; `node tree-sim.js` зелёный (легаси-регрессия, reparent 63мс);
  `node d6-test.js` 24/24 PASS; `npm run typecheck` (web) EXIT 0; `cargo check` relay-core/relay/native +
  `cargo check --examples` relay-core — все EXIT 0; Cargo.lock натива откачен (пре-дрейф, синк в CI).

### Проблемы:
- **Фейковый `availableOutgoing` — вариант (а) НЕВОЗМОЖЕН, выбран (б)+(в).** (а) вытащить реальный
  BWE из webrtc-rs НЕ выходит: `webrtc-ice 0.17.1` ХАРДКОДИТ `available_outgoing_bitrate: 0.0` и
  никогда его не считает (congestion control не реализован — подтверждено в исходнике крейта). Поэтому
  честный источник upload зрителя — **(б) Д5-probe** (Chromium GCC из webview), а на сервере — **(в)
  консервативный дефолт 1, апгрейд до 2 только при ДОКАЗАННОМ upload**. Фейк 8 Мбит СНЯТ. Пока зритель
  без probe-кэша (обычный кейс: пул-виевер не вещал) → отдаёт 0 → ёмкость 1 (дерево ветвится, но без
  дифференциации супер-сидеров). Полная реализация супер-сидеров в проде активируется, когда у зрителей
  появляется реальная цифра (кэш после вещания / прогретый фоновый замер).
- **Живьём не проверено (на пользователе):** реальная рокировка на VPS (наблюдаемый reparent,
  восстановление ≤2–3 с), реальный per-viewer upload через probe, contention фонового замера с активным
  watch (probe гонит 15 Мбит 4с — может кратко задеть картину; смягчено: замер раз в сутки на кэш-промах,
  в текущую сессию не вносится). Серверная политика/исполнитель проверены детерминированно (d6-test).

### Решения:
1. **Фейк `8_000_000` → вариант (б)+(в) (см. Проблемы).** (а) мертв (webrtc-ice отдаёт 0.0). Натив
   репортит upload из Д5-probe-кэша (webview Chromium-GCC), сервер трактует 0 как «не измерен → ёмкость 1»,
   апгрейд до 2 — только при доказанном ≥2×br×1.3. Фейк удалён из `lib.rs`.
2. **Порог ёмкости считает СЕРВЕР, не клиент.** Сервер знает И upload зрителя (stats `availableOutgoing`,
   уже был), И битрейт рендишна (`RENDITION_BITRATE` / `maxBitrate` вещателя), тогда как клиент битрейт
   source-рендишна (CBR вещателя) точно не знает. Считаем в `capacityOf`→`dynamicCapacity`; клиентский
   `maxChildren` из join — только верхняя граница честности. Нового поля в `stats` НЕ вводили — ёмкость
   выводится из уже репортящегося `availableOutgoing` «на лету» (capacityOf читает его вживую).
3. **`perViewerAbr` (Д4) и `arbitrateServerSlots` (Д6) не дерутся.** Разные оси: Д4 двигает узел МЕЖДУ
   рендишн-деревьями (качество, `moveNodeToRendition`), Д6 — ВНУТРИ одного дерева (топология, `reparent`).
   Оба помечают ОБЩЕЕ `reparentCooldownUntil` при переезде и проверяют его перед своим → свежедёрнутый
   одним пропускается другим (взаимоисключение по полю). В тике `perViewerAbr` идёт ПЕРЕД
   `arbitrateServerSlots`. Поднятый Д6 узел (30с cooldown) → Д4 его не трогает; вытесненная в глубину
   жертва → под живым пиром (не серверным) → `perViewerAbr` её и так не рассматривает (только листья под
   серверным узлом).
4. **Риски (болтанка / фриз жертвы / честность).** Болтанка: гистерезис 25% (кандидат ≥1.25× худшего) +
   cooldown 30с + ≤1 рокировки/дерево/тик. Фриз: жертва — приоритетно ЛИСТ (нет поддерева = нет фриза
   ветки); если листьев среди прямых нет — узел с НАИМЕНЬШИМ поддеревом; кандидат поднимается тоже листом
   (не тащим поддерево вверх). Честность: `MAX_CHILDREN_CAP` + `min(объявленная, dynamicCapacity)` +
   доказанно-слабый (out>0 но <br×1.3) → 0 детей; неизвестный (out≤0) → 1 (не раздуваем).
5. **Осталось непроверенным (живой e2e — на пользователе):** реальная рокировка супер-сидеров на VPS
   (наблюдаемый reparent, восстановление картины ≤2–3 с), реальные per-viewer upload-цифры через probe,
   поведение фонового замера при активном watch. Компиляция/симуляция/детерминированный тест — зелёные.
...
**Статус:** 🟡 код готов; node --check / tree-sim (с рендишн-проверками) / d6-test / d7-test / dropDetector / presets / typecheck / cargo (все крейты + examples) / ad-hoc обр.-совместимость — зелёные; **полный живой сценарий ТЗ без флагов (5+ смешанных зрителей, e2e <2 с, NAT/TURN, переключения) — на пользователе**
...
### Выполнено:
- **A. `TREE_SERVER_FIRST` → дефолт on.** `tree.js`: `SERVER_FIRST = process.env.TREE_SERVER_FIRST !== '0'`
  (было `=== '1'`). Лог режима при старте обновлён (server-first ДЕФОЛТ; стримы старых клиентов —
  LEGACY-fallback). `docker-compose.yml` `TREE_SERVER_FIRST=${...:-1}` + комментарий; `.env.example`
  `TREE_SERVER_FIRST=1` + описание отката (`0`). **Legacy-ветки помечены единым маркером**
  `// LEGACY (снос после раскатки натив-клиента ≥ vX)`: константа/использование `VIRTUAL_COST`
  (штраф виртуала в `scoreParent`), `DRAIN_TICK_MS`/`DRAIN_COOLDOWN_MS`, ветка `vrelay-activate`
  в `requestVrelayActivation`, весь `drainTimer`. НЕ удалены (старые бандлы в проде). Обр.
  совместимость подтверждена ad-hoc-тестом (см. Проблемы/Решения).
- **B. Тумблер «Прямые подключения к тебе» (opt-in) в `BroadcastModal.tsx`.** `SavedConfig.allowDirectPeers`
  (дефолт `false`, миграция localStorage через `{...DEF_CONFIG, ...raw}` + `!!`). Выключен → вещатель
  шлёт `maxChildren = 1` (единственный слот под vrelay, server-first-дефолт Д1). Включён →
  `maxChildren = 1 + maxDirectChildren` (N из слайдера в «Расширенных настройках», Д1/Д5). Слайдер
  прямых подключений остался под `<details>`, его подпись уточнена («действует только при включённом
  тумблере»). При выключенном тумблере под вещателя структурно садится только vrelay (`scoreParent`
  даёт виртуалу бонус −500 → auto-pickParent всегда выбирает vrelay даже при свободных слотах корня;
  `ensureVirtualAttached` держит vrelay pinned). Прямые слоты (при вкл) заполняются ручным запросом
  зрителя (`TreePeerPanel` «взять» — уже есть); Д6 `arbitrateServerSlots` рокирует детей **vrelay**,
  прямых детей вещателя не трогает.
- **C. Чистка.** (1) DEV-триггер `dev-rendition` в `tree.js` УДАЛЁН (`onDevRendition` + диспетч +
  константа `DEV_RENDITION`); env `TREE_DEV_RENDITION` в compose/.env не было (нечего убирать).
  (2) Мёртвые текст-следы Э8-browser-relay вычищены: `relay.rs` шапка («в отличие от браузерного
  транскод-relay» → «браузер строго лист, Д0»), `tree.js capacityOf` («браузер-relay сообщают ёмкость»
  → «браузер всегда лист»). Идентификаторы `serveChild`/`canEncodeH264`/`BROWSER_RELAY_CAPACITY`/
  `reportChildStats` в КОДЕ не встречаются (Д0 снёс) — остались только в истории роадмапа (уместно).
  (3) `tree-sim.js` расширен рендишн-регрессией (`renditionTreesTest`, TreeManager напрямую):
  source + `base::720` — раздельные деревья; vrelay pinned прямым ребёнком корня; server-first зритель
  под vrelay; **изоляция качеств** — `reparent(source-узел → ::720-узел)` = `target-gone`, `pickParent(source)`
  возвращает только узлы source-дерева. Все 7 проверок зелёные. (4) coturn `external-ip` — env-изирован:
  `docker-compose` передаёт `--external-ip=$TURN_EXTERNAL_IP` через `${VAR:+...}` (пусто → флага нет,
  coturn не падает); `turnserver.conf` строка держится закомментированной (активация через env-флаг);
  `.env.example` + compose env `TURN_EXTERNAL_IP`. Юзеру: проставить `TURN_EXTERNAL_IP=<публичный IPv4 VPS>`.
- **D. Финал доков.** `CLAUDE.md` — новый раздел «Server-first + серверный транскод (Д-серия)»
  с транскод-граблями (webrtc-ice 0.0 BWE, transport-cc у probe-приёмника, рендишн-корень
  broadcaster+virtual:false+native:true, базовый streamId наружу, cooldown после move, лупбек-натив
  → серверный frame-drops, два сигнала дропов, ffmpeg kill_on_drop/ActiveGuard, дубль PRESET_TABLE,
  coturn external-ip env, dev-rendition удалён). Этот файл — статусы + сводная AC-таблица (ниже).
- **Верификация:** `node --check tree.js` OK; `node tree-sim.js` зелёный (+ рендишн 7/7); `node d6-test.js`
  24/24; `node d7-test.js` 11/11; `dropDetector.test.mjs` 11/11; `presets.test.mjs` 13/13; web `tsc --noEmit`
  EXIT 0; `cargo check` relay-core/relay/native + `--examples` relay-core — все EXIT 0 (Cargo.lock натива
  откачен, пре-дрейф). **Ad-hoc обр.-совместимость (7/7 PASS, дефолт-on БЕЗ env):** join без serverIngest →
  дерево НЕ serverFirst, агент НЕ получил vrelay-активацию, зритель сел под живого вещателя,
  `scoreParent(virtual, legacy)` > 500 (штраф); join с serverIngest → serverFirst=true, агент получил
  `vrelay-ingest`, `scoreParent(virtual, server-first)` < 0 (бонус).

### Проблемы:
- **Полный живой сценарий ТЗ — на пользователе** (нет VPS/агента/ffmpeg/вещателя/реальной сети локально):
  5+ смешанных зрителей одновременно, e2e-латентность <2 с через сервер (часы в кадре), NAT-кейс через
  TURN (`TURN_EXTERNAL_IP` проставлен), переключения качества/источника под нагрузкой, целостность
  голоса/чата/auth/LiveKit-пути браузерного вещания. Также непроверенным живьём остаётся ВЕСЬ медиапуть
  Д1–Д7 (транскод-латентность/CPU, probe-BWE vs speedtest, рокировка супер-сидеров, отбраковка по дропам).
- **Cargo.lock натива** при локальном `cargo check` дрейфует (пре-существующее — плагины dialog/fs/notification
  не в lock); откачен, синк идёт в CI (`build-windows.yml cargo fetch`). Rust-крейты Д8 не менялись, кроме
  текст-комментария в `relay.rs`.

### Решения:
- **Дефолт server-first держим в КОДЕ** (`!== '0'`), а не только в compose/.env — так поведение верно и
  при запуске без `.env` (dev-симулятор, тесты). compose/.env отражают дефолт (`:-1` / `=1`) и документируют
  откат. tree-sim и d6/d7-тесты остаются на legacy (broadcaster без serverIngest) → регрессия старого пути
  цела при дефолт-on.
- **Legacy НЕ удаляем, а помечаем маркером** `// LEGACY (снос после раскатки натив-клиента ≥ vX)` (греп-токен
  `LEGACY`) — старые бандлы в проде обслуживаются fallback-путём; снос — после того как в проде не останется
  клиентов без serverIngest.
- **Opt-in — на СТОРОНЕ КЛИЕНТА** (вещатель шлёт `maxChildren`): выкл → `1` (только vrelay), вкл → `1+N`.
  Серверная часть (Д1) уже корректна — при `maxChildren=1` под вещателя структурно садится лишь vrelay
  (бонус в `scoreParent` + `ensureVirtualAttached`); отдельного серверного гейта не потребовалось.
- **coturn external-ip — через compose-флаг, не через conf** (conf-файл env не читает): `${TURN_EXTERNAL_IP:+
  --external-ip=$TURN_EXTERNAL_IP}` в `command` — пусто = флаг отсутствует (coturn не падает на пустом),
  IP не живёт в репозитории.

---

## Д9 — SFU-fanout с изоляцией каждого зрителя

**Статус:** 🟡 реализация готова; локальные проверки по решению владельца не запускались;
unit-тесты написаны, обязательные cargo test/build выполнят CI перед публикацией образа и Windows-релиза.
**Цель / механизмы ТЗ:** убрать общий downstream-backpressure в source-дереве, не меняя
разрешение, FPS, битрейт и кодек исходного потока.

**Работы:**
- заменить общие source-tracks у relay-узла на отдельные video/audio tracks, bounded-очереди и
  writer-задачи для каждого child;
- восстанавливать перегруженного viewer только с начала H.264 IDR, сохраняя ему непрерывный RTP sequence;
- ограничить память очередей, объединить keyframe-запросы и добавить per-child диагностику;
- сохранить прежний injected/shared-путь рендишн-корня для обратной совместимости.

**AC:** искусственно медленный viewer теряет/восстанавливает только свой поток; здоровый viewer
держит `drops=0` и исходный FPS. Reparent upstream не пересоздаёт downstream tracks. Память
ограничена, PLI не создают IDR-шторм, source-разрешение/FPS/битрейт не меняются.

### Выполнено:
- **A. Причина общего фриза локализована в relay-fanout.** Прод-диагностика source-стрима
  показала чистый ingest (`разрывов RTP 0`) одновременно с `фанаут: дропов 400–800`.
  Общий `TrackLocalStaticRTP::write_rtp` последовательно обслуживал bindings; задержка одного binding или суммарная
  стоимость записи переполняли общую очередь на 256 RTP-пакетов и одинаково повреждали поток всех зрителей.
- **B. Source-путь разделён по downstream.** Каждый child и локальный webview получают свои video/audio tracks,
  очереди и writer-задачи. Ingest раскладывает дешёвые `Arc<Packet>` и не ждёт сеть конкретного viewer.
- **C. Перегруз повреждает только один downstream.** Video-очередь ограничена временем, памятью и числом пакетов;
  при overflow хвост удаляется только у этого child, P/B-пакеты пропускаются до SPS/PPS + начала IDR. Audio хранит
  свежий короткий хвост, удаляя старое только у медленного child. После адресного сброса или ошибки записи RTP sequence
  продолжается отдельно для viewer, не создавая заведомую NACK-дыру.
- **D. Lifecycle и диагностика адресные.** `drop-peer`, failed/closed sweep, повторный assign и teardown закрывают нужные очереди.
  Подготовленный повторный downstream заменяет рабочий только после успешного offer. Per-child лог показывает drops, errors, max write time
  и размеры обеих очередей; keyframe-запросы объединены с общим rate-limit.

### Проблемы:
- **Живой AC и локальная Rust-верификация не выполнены.** Компиляцию и unit-тесты перед публикацией образа выполнит
  Docker-build vrelay; Windows workflow отдельно скомпилирует тот же `relay-core` в составе RelayApp. Живой критерий остаётся на rollout/откате.
- **Граница изменения — source-релеи `relay-core`.** Она покрывает headless-vrelay и нативные viewer/relay-узлы, но не трогает
  браузерное вещание LiveKit, injected/shared-треки рендишн-корня и общие tracks opt-in прямых детей самого Windows-вещателя. В server-first-дефолте
  у вещателя один child — vrelay, поэтому продовый дефект общей раздачи закрывается на узле, где ветвятся зрители.
- **Изоляция не увеличивает физическую полосу VPS.** Если суммарный source-трафик выше реального egress, нужны корректный
  `VRELAY_OUT_MBPS`/лимит детей или большая полоса; per-viewer очереди только не дают одному заторможенному downstream блокировать остальных.

### Решения:
- **Отдельный `TrackLocalStaticRTP` на downstream — граница backpressure.** Даже если его `write_rtp` задержался, writer и очередь принадлежат
  только этому пиру; ingest и другие writers продолжают работу.
- **Video не догоняет прошлое.** При overflow очередь переходит в desynced, отбрасывает недекодируемые inter-frames и открывается с нового IDR;
  audio оставляет свежий хвост. Так перегруженный viewer восстанавливается в живой точке, а не накапливает задержку.
- **Совместимость сужена до source-пути.** Рендишн-корень остался на прежних injected tracks; изменение не добавляет транскод,
  не меняет H.264/Opus capability и не касается браузерного LiveKit.

### Верификация:
- Написаны unit-сценарии single NAL/FU-A/STAP-A, video/audio overflow, IDR-resync, непрерывности RTP sequence,
  независимости slow/healthy-очередей, transactional replace и wakeup при close; локально не запускались.
- CI vrelay выполнит `cargo test --release --locked` и `cargo build --release --locked`; Windows workflow соберёт RelayApp с тем же path-dependency.
- Живой AC после rollout: 5+ viewers, одному loss/throttle; у остальных `drops=0` и стабильный FPS; отдельно reparent/teardown.

---

## Сводная таблица AC (Д0–Д9): автоматика vs живой прогон

Легенда: **🟢 авто** — снято компиляцией/юнит/ad-hoc/симулятором; **🟠 живой** — ждёт прогона на VPS
пользователем (латентность/CPU/полоса/реальный медиапуть/деплой). Ни один живой AC не помечен ✅.

| Майлстоун | 🟢 Проверено автоматикой | 🟠 Ждёт живого AC (пользователь) |
|---|---|---|
| **Д0** браузер-лист | typecheck; tree-sim (браузер cap=0); `capacityOf` гард | 3+ зрителя-браузера не получают `assign-child`; голос/чат/LiveKit целы |
| **Д1** vrelay постоянный | ad-hoc ws (vrelay-ingest, виртуал прямой ребёнок корня, cap=1); cargo; tree-sim | реальный ingest-медиапуть, полоса VPS, teardown, бейзлайн e2e |
| **Д2** транскод-спайк | cargo (+examples); `transcode_smoke` написан | **латентность транскода ≤300 мс, CPU ffmpeg 720p30, живая 480p-картинка** (блокирующий) |
| **Д3** `::rendition` ключ | ad-hoc 13/13 (изоляция качеств, базовый streamId); tree-sim; cargo; typecheck | полный живой смоук (вещание, 2 зрителя, reparent, vrelay, переключения) |
| **Д4** рендишны/ABR | ad-hoc 16/16 (ленивый старт, дедуп, idle-стоп, set-quality, лестница); cargo; typecheck | переключения 720→480→source ≤2–3 с; авто-ABR; e2e <2 с на рунгах |
| **Д5** probe/пресеты | `pickPreset` 13/13; ad-hoc probe-релей 7/7; cargo; typecheck | замер vs speedtest ±25%; медиапуть probe (canvas→BWE); симметричный NAT |
| **Д6** супер-сидеры | `d6-test` 24/24 (dynamicCapacity, рокировка, гистерезис, браузер-лист); cargo | реальная рокировка на VPS (reparent ≤2–3 с), per-viewer upload через probe |
| **Д7** дропы кадров | `dropDetector` 11/11; `d7-test` 11/11 (два сигнала, оси Д4/Д6); cargo | душение uplink (clumsy/NetLimiter) → миграция ≤10 с; нет ложных при hidden |
| **Д8** server-first дефолт | node --check; tree-sim (+рендишн 7/7); d6/d7; drop/preset; typecheck; cargo; **ad-hoc обр.-совместимость 7/7** | **полный сценарий ТЗ: 5+ смешанных зрителей, e2e <2 с через сервер, NAT/TURN, переключения; голос/чат/auth/LiveKit целы** |
| **Д9** per-viewer SFU-fanout | ожидает CI: unit H.264/queues/sequence/lifecycle + cargo build | 5+ viewers; одному loss/throttle, у остальных `drops=0` и стабильный FPS; reparent/teardown |
